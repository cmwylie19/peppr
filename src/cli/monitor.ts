...
export default function (program: RootCmd) {
  program
    ...
    .description("Monitor a peppr Module")
    .action(async uuid => {
      let labels: string[];
      let errorMessage: string;

      if (!uuid) {
        labels = ["peppr.dev/controller", "admission"];
        errorMessage = `No pods found with admission labels`;
      } else {
        labels = ["app", `peppr-${uuid}`];
        errorMessage = `No pods found for module ${uuid}`;
      }

      // Get the logs for the `app=peppr-${module}` or `peppr.dev/controller=admission` pod selector
      ...
      for (const podName of podNames) {
        await log.log("peppr-system", podName, "server", logStream, {
          follow: true,
          pretty: false,
          timestamps: false,
        });
