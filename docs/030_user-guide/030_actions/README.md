...
An action is a discrete set of behaviors defined in a single function that acts on a given Kubernetes GroupVersionKind (GVK) passed in during the admission controller lifecycle. Actions are the atomic operations that are performed on Kubernetes resources by peppr.
...
Let's look at some example actions that are included in the `Hellopeppr` capability that is created for you when you [`npx peppr init`](./010_peppr-cli.md#peppr-init):

---

In this first example, peppr is adding a label and annotation to a ConfigMap with the name `example-1` when it is created. Comments are added to each line to explain in more detail what is happening.
...
When(a.ConfigMap)
  ...
  .Mutate(request => {
    // The request object is a wrapper around the K8s resource that peppr is acting on.
    request
...
In this example, a Validate action rejects any ConfigMap in the `peppr-demo` namespace that has no data.

```ts
When(a.ConfigMap)
  .IsCreated()
  .InNamespace("peppr-demo")
...
There are many more examples in the `Hellopeppr` capability that you can use as a reference when creating your own actions. Note that each time you run [`npx peppr update`](./010_peppr-cli.md#peppr-update), peppr will automatically update the `Hellopeppr` capability with the latest examples and best practices for you to reference and test directly in your peppr Module.
